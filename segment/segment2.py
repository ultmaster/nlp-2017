...
def evaluate(text, segs):
    ...
    for word in set(words):
        word_size += pow(WORDLIST[word], 0.3)
    return text_size * 3 - word_size * 0.3
...
def segment_split(text, cooling_rate):
    seg1 = "0" * len(text)
    seg1 = flip_n(seg1, len(text) // 6)
    seg1 = anneal(text, seg1, 5000, cooling_rate)
    return segment(text, seg1)
